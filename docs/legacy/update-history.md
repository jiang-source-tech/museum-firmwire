...
> 本文件是历史变更日志，不是当前产品规格。旧项目名称和已删除能力只作为迁移记录保留；当前实现以 `docs/museum-firmware-roadmap.zh-CN.md`、OTA/串口文档和源码为准。

## 2026-08-09 00:00:00 +08:00

### Waveshare ESP32-S3 Touch LCD 1.46 切换为博物馆单页界面

#### 修改内容

- 删除 Home / Notifications / Overview 三态分页状态机、卡片层、上下滑切换、通知卡片滚动/清理动画和总览页渲染。
- 删除 `xiaoxin_card_pager.c/.h` 与 `xiaoxin_overview_model.c/.h`，并从目标板构建源中移除总览模型。
- 主页触摸只保留设置、电源和语音交互；纵向拖动不再打开旧分页。
- `ShowNotification()` 与 `UpsertNotification()` 改为复用博物馆主界面的临时通知文字；语音通道、设置页、低功耗时钟和电池保护保持不变。
- `notify_test` 保留为临时通知检查命令，不再打开通知页。
- 删除分页与总览模型专用测试，将板级检查收敛为单页触摸、临时通知桥接和构建源清理三个行为。

#### 验证结果

- `python -m pytest tests/xiaoxin_notification_visual_path_test.py tests/museum_serial_debug_command_test.py tests/xiaoxin_settings_path_test.py -p no:cacheprovider -q`：历史记录，需按当前工作区重新执行。
- ESP-IDF 增量构建通过，生成 `build/ai_pet.bin`；二进制大小 `0x564740`，最小应用分区剩余 `0x9b8c0`，约 `10%`。
- `git diff --check`：通过，仅有仓库既有 CRLF 换行提示。
- 尚未烧录实机；需要确认上下滑不再切页、主页临时通知可见、设置页和桌宠触摸仍正常。
