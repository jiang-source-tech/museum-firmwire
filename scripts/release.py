...
def ensure_ota_data_artifact() -> None:
    """Build ESP-IDF's blank OTA selection data before consuming flash_args.

    `idf.py build` deliberately does not depend on ``blank_ota_data`` while
    ``flash`` does.  ``merge-bin`` consumes the same flash arguments as
    ``flash``, so without this step an OTA layout can have a dangling
    ``ota_data_initial.bin`` entry.
    """
    flasher_args_path = Path("build/flasher_args.json")
    try:
        with flasher_args_path.open(encoding="utf-8") as source:
            flasher_args = json.load(source)
    except (OSError, json.JSONDecodeError) as error:
        print(f"Failed to read {flasher_args_path}: {error}", file=sys.stderr)
        sys.exit(1)

    flash_files = flasher_args.get("flash_files")
    if not isinstance(flash_files, dict):
        print(f"{flasher_args_path} has no flash_files object", file=sys.stderr)
        sys.exit(1)

    needs_ota_data = any(
        isinstance(path, str) and path.replace("\\", "/").endswith("ota_data_initial.bin")
        for path in flash_files.values()
    )
    if needs_ota_data and os.system("idf.py blank_ota_data") != 0:
        print("blank_ota_data failed", file=sys.stderr)
        sys.exit(1)


def merge_bin() -> None:
    ensure_ota_data_artifact()
