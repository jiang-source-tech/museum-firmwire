...
struct DeviceStateTransition {
    DeviceState old_state = kDeviceStateUnknown;
    DeviceState new_state = kDeviceStateUnknown;
    bool accepted = false;
...
public:
    ...
    /**
     * Atomically validate and commit a state without logging or notifying
     * listeners. PublishTransition must be called after external locks are
     * released.
     */
    DeviceStateTransition CommitTransition(DeviceState new_state);
    bool PublishTransition(const DeviceStateTransition& transition);
