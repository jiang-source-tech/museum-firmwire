...
void LcdDisplay::SetupUI() {
    ...
    /* chat_message_label_ is wider than the clipped bar and moves left once. */
    chat_message_label_ = lv_label_create(bottom_bar_);
    lv_label_set_text(chat_message_label_, "");
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    SetChatMessageInternal(role, content, false);
}

void LcdDisplay::UpdateChatMessage(const char* role, const char* content) {
    SetChatMessageInternal(role, content, true);
}

void LcdDisplay::SetChatMessageInternal(
    const char* role,
    const char* content,
    ...
    const int32_t previous_x = preserve_scroll_position
        ? lv_obj_get_x(chat_message_label_)
        : 0;
    ...
    const int32_t text_width = lv_obj_get_width(chat_message_label_);
    const int32_t end_x = std::min<int32_t>(0, viewport_width - text_width);
    if (end_x == 0) {
        lv_obj_set_x(chat_message_label_, (viewport_width - text_width) / 2);
        return;
    }

    const int32_t start_x = preserve_scroll_position
        ? std::clamp<int32_t>(previous_x, end_x, 0)
        : 0;
    lv_obj_set_x(chat_message_label_, start_x);
    if (start_x > end_x) {
        lv_anim_t animation;
        ...
        if (!preserve_scroll_position) {
            lv_anim_set_delay(&animation, 1000);
        }
