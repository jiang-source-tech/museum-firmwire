...
int NoAudioCodec::Write(const int16_t* data, int samples) {
    ...
    const size_t expected_bytes = samples * sizeof(int32_t);
    const esp_err_t err = i2s_channel_write(
        tx_handle_,
        buffer.data(),
        expected_bytes,
        &bytes_written,
        pdMS_TO_TICKS(kNoAudioCodecWriteTimeout.count()));
    if (err != ESP_OK || bytes_written != expected_bytes) {
        ESP_LOGE(
            TAG,
            "I2S output write failed: err=%s bytes=%u expected=%u timeout_ms=%lld",
            esp_err_to_name(err),
            static_cast<unsigned>(bytes_written),
            static_cast<unsigned>(expected_bytes),
            static_cast<long long>(kNoAudioCodecWriteTimeout.count()));
