...
#include "audio_pipeline_epoch.h"
#include "audio_playback_failure.h"
#include "audio_output_watchdog.h"
...
struct AudioServiceCallbacks {
    ...
    std::function<void(uint32_t, AudioPlaybackFailureReason)> on_playback_failure;
...
public:
    ...
    AudioPlaybackDrainResult WaitForPlaybackDrainResult(
        uint32_t generation,
...
private:
    ...
    AudioPipelineEpoch audio_pipeline_epoch_;
    ...
    bool audio_decode_busy_ = false;
    bool audio_output_busy_ = false;
    uint64_t audio_output_sequence_ = 0;
    AudioPlaybackFailureState playback_failures_;
    ...
    AudioPlaybackFailureReason SetDecodeSampleRate(
        int sample_rate,
        int frame_duration);
