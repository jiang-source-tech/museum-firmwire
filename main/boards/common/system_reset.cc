...
void SystemReset::CheckButtons() {
    if (gpio_get_level(reset_factory_pin_) == 0) {
        ESP_LOGI(TAG, "Button is pressed, reset persistent data");
        ResetPersistentDataAndRestart();
    }
...
void SystemReset::ResetPersistentDataAndRestart() {
    ESP_LOGI(TAG, "Resetting persistent data while preserving the verified firmware");
    ResetNvsFlash();
    RestartInSeconds(3);
}
